...
@mixin grid-base($widget-name) {
  ...
  .dx-#{$widget-name}-rowsview {
    ...
    .dx-row-removed:not(.dx-row-lines) > td {
      ...
      border-bottom: 1px solid $datagrid-hover-bg;
    }
    ...
    &.dx-#{$widget-name}-sticky-columns {
      .dx-data-row:not(.dx-row-lines, .dx-edit-row, .dx-selection, .dx-row-focused) > td:not(.dx-editor-cell, .dx-master-detail-cell) {
